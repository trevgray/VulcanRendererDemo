...
layout (location = 5) in vec2 fragTextureCoords;
...
void main() { 
	...
	vec4 kd[3] = {vec4(0.4, 0.1, 0.1, 0.0), vec4(0.1, 0.1, 0.4, 0.0), vec4(0.1, 0.4, 0.1, 0.0)}; //colour of light - const means it cannot be changed just like C++
	vec4 ka[3] = {0.01 * kd[0], 0.01 * kd[1], 0.01 * kd[2]}; //ambient light
